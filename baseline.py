...
def visualize_data(data_dir, batch_size):
    data_dir = pathlib.Path(data_dir)
    train_ds = tf.keras.preprocessing.image_dataset_from_directory(data_dir)
      
    class_names = train_ds.class_names 

    # 훈련 데이터셋의 처음 5개의 이미지 시각화하기
    ...
    for images, labels in train_ds.take(1):
        for i in range(6):
            ...
            plt.axis("off")
            plt.savefig('savefig/data_example_baseline.png')
...
def main(args):
    # Build dataset
    root = f'{args.root}/crop_data_labeled_class6'
    
    train_transform = T.Compose([
        T.ToTensor(),
        T.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
    ])
    train_data = Labeled_data(root, train=True, transform=train_transform)
    ...
    val_data = Labeled_data(root, train=False, transform=val_transform)
    val_loader = DataLoader(val_data, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)

    # visualize data
    train_root = f'{root}/train'
    visualize_data(train_root, args.batch_size)
